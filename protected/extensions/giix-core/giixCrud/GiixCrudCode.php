...
class GiixCrudCode extends CrudCode {
	...
	{
		$class = 'control-label';
		if ($column->isForeignKey || strtoupper($column->dbType) == 'TINYINT(1)')
			$class = 'lbl_c';
	...
	public function generateActiveField($modelClass, $column) {
		if ($column->isForeignKey) {
			...
			return "echo \$form->dropDownList(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true)), array('class'=>'span12'))";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "echo \$form->checkBox(\$model, '{$column->name}', array('class'=>'checkbox'))";
		} else if (strtoupper($column->dbType) == 'DATE') {
			return "\$form->widget('zii.widgets.jui.CJuiDatePicker', array(
			'model' => \$model,
			'attribute' => '{$column->name}',
			'value' => \$model->{$column->name},
			'htmlOptions'=>array('class'=>'span6 datepicker_s hasDatepicker'),
			'options' => array(
			...
			return "echo \$form->textArea(\$model, '{$column->name}', array('cols'=>'30', 'rows'=>'10'))";
		} else {
			...
			if ($column->type !== 'string' || $column->size === null)
				return "echo \$form->{$inputField}(\$model, '{$column->name}', array('class'=>'span12'))";
			else
				return "echo \$form->{$inputField}(\$model, '{$column->name}', array('class'=>'span12', 'maxlength' => {$column->size}))";
